...
def _build_library(root: str) -> int:
    ...
    for _ in range(40):
        img = Image.new(
            "RGB", (800, 600), (random.randrange(255), random.randrange(255), random.randrange(255))
        )
        for _ in range(30):
    ...
    for i in range(N):
        ...
        if existing and random.random() < DUP_RATIO:
            shutil.copy2(
                os.path.join(sub, random.choice(existing)), os.path.join(sub, f"img{i}_copy.jpg")
            )
        else:
            bases[i % 40].save(
                os.path.join(sub, f"img{i}.jpg"), quality=random.choice([70, 85, 95])
            )
        made += 1
    return made
...
def main() -> None:
    ...
    try:
        ...
        size_mb = (
            sum(os.path.getsize(os.path.join(r, f)) for r, _, fs in os.walk(root) for f in fs) / 1e6
        )
        ...
        runs = [
            (
                ...
                AppConfig(
                    analyze_images=True,
                    detect_pixel_identical=True,
                    analyze_similar_images=False,
                    use_cache=False,
                    workers=cpu,
                ),
            ),
        ...
        t = time.time()
        r = run_analysis(root, cfg, incremental=True)
        print(
            f"[incremental  ] {time.time() - t:6.1f}s   cache_hits={r.cache_hits}  "
